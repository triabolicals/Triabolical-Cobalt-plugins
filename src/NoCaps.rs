...
pub fn set_job_caps(){
    unsafe {
        ...
        for x in 0..job_count {
            ...
            for i in 0..10 { 
                cap.array.m_item[i] = 127 + base.array.m_item[i];
             }
...
pub fn return999(offset: usize){
    Patch::in_text(offset).bytes(&[0xE0, 0x7C, 0x80, 0x52]);
    Patch::in_text(offset+0x4).bytes(&[0xC0, 0x03, 0x5F, 0xD6]);
}
